...
static int
mks_display_get_surface_scale_factor (MksDisplay *self)
{
  GtkNative *native;
  GdkSurface *surface;
...
static void
mks_display_scale_factor_changed (MksDisplay *self,
                                  GParamSpec *pspec)
{
  g_assert (MKS_IS_DISPLAY (self));
  ...
  if (native != NULL && (surface = gtk_native_get_surface (native)))
    {
      ...
      priv->scale_handler = g_signal_connect_swapped (priv->surface,
                                                      "notify::scale-factor",
                                                      G_CALLBACK (mks_display_scale_factor_changed),
                                                      self);
    }
                           ...
                           int        baseline)
{
  ...
  if (priv->auto_resize)
    {
      scale_factor = mks_display_get_surface_scale_factor (self);
      device_width = width * scale_factor;
      device_height = height * scale_factor;
